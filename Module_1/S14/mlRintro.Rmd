...
BiocManager::install(c("MSnbsase", "pRoloc")) ## software
BiocManager::install("pRolocdata") ## date
```
...
#use the `iris` dataset with the sepal and petal length variables only 
...
# `clusters` - the cluster membership
...
#the distance between the clusters is proportional to the branch lengths.
...
#cut dataset at a distance around 1.5, with would produce 2 clusters
plot(hcr)
...
cutree(hcl, k = 3)
cutree(hcl, h = 3.9)
#verify that both provide the same results
...
#verify that the variables are of different scales
colMeans(iris[, 1:4])
hcl1 <- hclust(dist(iris[, 1:4]))
#scale the data
hcl2 <- hclust(dist(scale(iris[, 1:4])))
#compare the hierarchical clusters obtained on the original and scaled data
par(mfrow = c(1, 2))
plot(hcl1, main = "original data")
plot(hcl2, main = "scaled data")
#verify that scaled data leads to better result
table(iris$Species, cutree(hcl1, k = 3))
table(iris$Species, cutree(hcl2, k = 3))
```
